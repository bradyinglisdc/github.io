...
export class OpportunitiesPageSetup {
    ...
    static signUpOpportunity() {
        ...
        if (Opportunity.signUp("1", fullNameInput.value.trim(), emailInput.value.trim(), preferredRoleInput.value.trim())) {
    ...
    static sortOpportunities() {
        ...
        const stringArray = opportunitiesJSON.opportunities.map((opportunity) => opportunity.opportunity.title);
        ...
        for (const title of sortedArray) {
            const matchedOpportunity = opportunitiesJSON.opportunities.find((opp) => opp.opportunity.title === title);
...
//# sourceMappingURL=opportunitiesPageSetup.js.map